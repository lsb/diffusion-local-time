...
pipe(prompt="a clock", image=mask_image(timestamp=datetime.now()), num_inference_steps=4, guidance_scale=7.0, controlnet_conditioning_scale=1.5).save(target_filename)
...
fps = 30
iteration_range = range(fps * (4*3600 + 19*60), 86400 * fps)
...
#    image = current_mask_image.convert("RGB")
    image = pipe(
        ...
        generator=torch.manual_seed(int(synthetic_time.timestamp()) // 60),
        height=image_size[1],
        width=image_size[0],
    ).images[0]
    if False:
